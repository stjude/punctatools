...
process.image.file=function(cell.data,      # cell data data.frame
                            ...
                            cell.list.ex,   # cells (mitotic/ dead) to exclude in excel file (optional)
...
dir0 = "/home/stripa76/Research/Puncta_Thermo_Calc.CBI/data_processing/" # define directory

process.image.file(cell.data = read.csv(paste0(dir0, "input/", "/cell_stats.csv")),
           puncta.data = read.csv(paste0(dir0, "input/", "puncta_stats.csv")),
           #cell.list.ex = read.csv(paste0(dir0, "input/", "cells_to_exclude.csv")), #(optional)
...
# write the puncta features into an excel file (optional)
write.xlsx(df, file = paste0(dir0, "output/", "puncta_features", ".xlsx"))
```
...
#plot1[["puncta.number.per.nuclear.vol.GFP"]] + ylab(~"Puncta # (/"~10^3~mu*m^3~")") + scale_y_log10()
#plot1[["avg.puncta.vol.GFP"]] + ylab(V[p]~"("~mu*m^3~")") + scale_y_log10()
#plot1[["light.phase.conc.GFP"]] + ylab(~"[LP] ("~mu*M~")") + scale_y_log10(limits = c(0.02,110))
#plot1[["puncta.dense.phase.conc.GFP"]] + ylab(~"[DP] ("~mu*M~")") + scale_y_log10(limits = c(0.02,110))
#plot1[["Kp.GFP"]] + ylab(expression(K[p])) + scale_y_log10()
```
...
#plot2[["DeltaGtr.GFP"]] + ylab(expression(Delta~G^{tr}~"("~kcal~mol^{-1}~"/cell)"))
```

```{r, fig.width=12, fig.height=6}
library(patchwork)

pdf(file = paste0(dir0, "output/", "puncta_XY_plots", ".pdf"), width=12, height=6)
...
p6<-plot2[["DeltaGtr.GFP"]] + ylab(expression(Delta~G^{tr}~"("~kcal~mol^{-1}~"/cell)"))
