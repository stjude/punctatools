...
```{r}
# write the puncta features into a .csv file
write.csv(df, file = paste0(dir0, "puncta_thermo_features", ".csv"), row.names = F)
```
...
p3<-plot1[["light.phase.conc.GFP"]] + ylab(~"[LP] ("~mu*M~")") + scale_y_log10()
p4<-plot1[["puncta.dense.phase.conc.GFP"]] + ylab(~"[DP] ("~mu*M~")") + scale_y_log10()
p5<-plot1[["Kp.GFP"]] + ylab(expression(K[p])) + scale_y_log10()
p6<-plot2[["DeltaGtr.GFP"]] + ylab(expression(Delta~G^{tr}~"("~kcal~mol^{-1}~")"))

png(file = paste0("../../docs/", "puncta_thermo", ".png"), width=12, height=6, units="in", res=300)
